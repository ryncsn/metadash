      ...
      <div v-for="value in plugins" class="panel panel-default">
        ...
        <div class="panel-body">
          <form class="form-horizontal">
            <div v-for="config in value.configs" :key="config.key" class="form-group" v-bind:class="{ 'has-error': errors[config.key] }">
              ...
              <div class="col-sm-10">
                ...
                <span v-if="errors[config.key]" class="help-block">{{ errors[config.key] }}</span>
              </div>
            </div>
          </form>
        </div>
    ...
    save: _.debounce(function () {
      ...
      changedConfigs.map((config) => {
        this.$http.put('/api/configs/' + config.key, {
          ...
          this.errors[config.key] = 'Failed to save'
          this.configs = JSON.parse(JSON.stringify(this.savedConfigs))
        })
      })
    }, 500),
    load () {
      ...
      handler (newConfig) {
        ...
        for (let config of changedConfigs) {
          if (!config.nullable && !config.value) {
            this.errors[config.key] = "Can't be empty"
          }
        }
